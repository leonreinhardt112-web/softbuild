    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <Tabs defaultValue="zuschlaege">
            <TabsList className="w-full">
              <TabsTrigger value="zuschlaege" className="flex-1">Zuschläge (Abschn. 2)</TabsTrigger>
              <TabsTrigger value="lohn" className="flex-1">Verrechnungslohn (Abschn. 1)</TabsTrigger>
            </TabsList>

            {/* ── Tab 1: Zuschläge ── */}
            <TabsContent value="zuschlaege" className="space-y-3 pt-2">
            ...
            {/* ── Tab 2: Verrechnungslohn ── */}
            <TabsContent value="lohn" className="space-y-3 pt-2">
              <div className="space-y-1 text-xs text-muted-foreground flex items-start gap-1.5 bg-muted/40 p-3 rounded-lg">
                <Info className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                <p>Der Verrechnungslohn (1.6) ergibt sich aus Mittellohn + Zuschlägen. Er fließt automatisch in den EFB 221 PDF-Export ein.</p>
              </div>
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-muted/50">
                    <th className="text-left font-medium text-muted-foreground py-2 px-2 border border-border w-8">#</th>
                    <th className="text-left font-medium text-muted-foreground py-2 px-2 border border-border">Position</th>
                    <th className="text-center font-medium text-muted-foreground py-2 px-2 border border-border w-28">Zuschlag %</th>
                    <th className="text-center font-medium text-muted-foreground py-2 px-2 border border-border w-28">€/h</th>
                  </tr>
                </thead>
                <tbody>
                  {/* 1.1 Mittellohn */}
                  <tr>
                    <td className="py-2 px-2 border border-border font-bold">1.1</td>
                    <td className="py-2 px-2 border border-border">
                      <div className="font-semibold">Mittellohn ML</div>
                      <div className="text-muted-foreground text-[11px]">einschl. Lohnzulagen u. Lohnerhöhung</div>
                    </td>
                    <td className="py-1.5 px-2 border border-border" />
                    <td className="py-1.5 px-2 border border-border">
                      <div className="relative">
                        <Input type="number" min="0" step="0.01" value={values.ml_euro ?? 0}
                          onChange={e => set("ml_euro", e.target.value)}
                          className="h-7 text-right pr-7 text-xs w-full" />
                        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground">€</span>
                      </div>
                    </td>
                  </tr>
                  {/* 1.2 LGK */}
                  <tr>
                    <td className="py-2 px-2 border border-border font-bold">1.2</td>
                    <td className="py-2 px-2 border border-border">
                      <div className="font-semibold">Lohngebundene Kosten</div>
                      <div className="text-muted-foreground text-[11px]">Sozialkosten und Soziallöhne, als Zuschlag auf ML</div>
                    </td>
                    <td className="py-1.5 px-2 border border-border">
                      <div className="relative">
                        <Input type="number" min="0" step="0.5" value={values.lgk_pct ?? 0}
                          onChange={e => set("lgk_pct", e.target.value)}
                          className="h-7 text-right pr-7 text-xs w-full" />
                        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground">%</span>
                      </div>
                    </td>
                    <td className="py-2 px-2 border border-border text-right font-medium pr-3">
                      {ml > 0 ? (ml * lgkPct / 100).toFixed(2) : "–"}
                    </td>
                  </tr>
                  {/* 1.3 LNK */}
                  <tr>
                    <td className="py-2 px-2 border border-border font-bold">1.3</td>
                    <td className="py-2 px-2 border border-border">
                      <div className="font-semibold">Lohnnebenkosten</div>
                      <div className="text-muted-foreground text-[11px]">Auslösungen, Fahrgelder, als Zuschlag auf ML</div>
                    </td>
                    <td className="py-1.5 px-2 border border-border">
                      <div className="relative">
                        <Input type="number" min="0" step="0.5" value={values.lnk_pct ?? 0}
                          onChange={e => set("lnk_pct", e.target.value)}
                          className="h-7 text-right pr-7 text-xs w-full" />
                        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground">%</span>
                      </div>
                    </td>
                    <td className="py-2 px-2 border border-border text-right font-medium pr-3">
                      {ml > 0 ? (ml * lnkPct / 100).toFixed(2) : "–"}
                    </td>
                  </tr>
                  {/* 1.4 KL */}
                  <tr className="bg-muted/30">
                    <td className="py-2 px-2 border border-border font-bold">1.4</td>
                    <td className="py-2 px-2 border border-border">
                      <div className="font-semibold">Kalkulationslohn KL</div>
                      <div className="text-muted-foreground text-[11px]">(Summe 1.1 bis 1.3)</div>
                    </td>
                    <td className="py-2 px-2 border border-border" />
                    <td className="py-2 px-2 border border-border text-right font-bold text-primary pr-3">
                      {kl > 0 ? kl.toFixed(2) + " €" : "–"}
                    </td>
                  </tr>
                  {/* 1.5 Zuschlag auf KL */}
                  <tr>
                    <td className="py-2 px-2 border border-border font-bold">1.5</td>
                    <td className="py-2 px-2 border border-border">
                      <div className="font-semibold">Zuschlag auf Kalkulationslohn</div>
                      <div className="text-muted-foreground text-[11px]">(aus Zeile 2.4, Spalte 1 = {totalLohnZ.toFixed(1)} %)</div>
                    </td>
                    <td className="py-2 px-2 border border-border text-center font-medium text-primary">{totalLohnZ.toFixed(1)} %</td>
                    <td className="py-2 px-2 border border-border text-right font-medium pr-3">
                      {kl > 0 ? (kl * totalLohnZ / 100).toFixed(2) : "–"}
                    </td>
                  </tr>
                  {/* 1.6 VL */}
                  <tr className="bg-primary/5">
                    <td className="py-2 px-2 border border-border font-bold">1.6</td>
                    <td className="py-2 px-2 border border-border">
                      <div className="font-semibold">Verrechnungslohn VL</div>
                      <div className="text-muted-foreground text-[11px]">(Summe 1.4 und 1.5)</div>
                    </td>
                    <td className="py-2 px-2 border border-border" />
                    <td className="py-2 px-2 border border-border text-right font-bold text-primary pr-3 text-sm">
                      {vl > 0 ? vl.toFixed(2) + " €/h" : "–"}
                    </td>
                  </tr>
                </tbody>
